...
public class SharpTableModel extends DefaultTableModel {
	...
	for (int col = 1; col < numColumns; col++) {
	    Cell temp = new Cell(Node.translateColumn(col));
         // Cell temp = new Cell(sharp.fieldNames[col]);
	...
	*/
    ...
    public String getColumnName(int col) {
        ...
        if (col < SharpTools.baseCol) {
            ...
            if (sharp.hasFieldHeaders) {
                //   return String.valueOf(Node.translateColumn(col));
                return String.valueOf(sharp.fieldNames[col - 1]);
            } else {
                return String.valueOf(Node.translateColumn(col));
            }
        }
...
*/
	...
	/* if for some reason value out of bounds ignore */
        if (temp != null) {
            ...
            if (input instanceof Formula) {
                ...
                // is string
                // System.out.println(" setCellAt()   treat as normal data cel ");
                temp.setData(input); // added this
                updateRefs(aRow, aColumn);
            }
        }
    ...
    public void setRange(CellRange range, Object[][] data) {
        ...
        for (int i = range.getStartRow(); i <= range.getEndRow(); i++) {
            for (int j = range.getStartCol(); j <= range.getEndCol(); j++) {
                ...
                //  sharp.currentDBlabel.setText("Getting Result: " + j);
    ...
    public void updateRefs(int aRow, int aColumn) {
        ...
        while (it.hasNext()) {
            CellPoint point = (CellPoint) it.next();
            try {
                System.out.println(" updateRefs() getNumericValueAt.....  710");
    ...
    public void insertColumn(CellRange insertRange) {
        ...
        for (int i = 0; i < insertNum; i++) {
            ...
            //  newcol.setHeaderValue(sharp.fieldNames[curCol]);
            tm.addColumn(newcol);
        }
    ...
    private void addColumn() {
        ...
        /* Initialize the new column */
            ...
            System.out.println("ERROR: SharpTableModel [949]: " + ex.toString());
        }
	...
	*/
    ...
    public Number getNumericValueAt(int row, int col) throws ParserException {
        System.out.println("getNumericValueAt()   1065");
        Cell cell = getCellAt(row, col);
        if (cell != null) {
            ...
            if (type == Cell.FORMULA) {
                ...
                if (form.needsRecalc()) {
                    try {
                        ...
                        cell.setValue(value);
                        System.out.println("setValue()   1077");
                    } catch (ParserException e) {
                        cell.setValue(e);
                        System.out.println("setValue() ParserException");
                        value = e;
                    }
                }

                if (value instanceof ParserException) {
                    System.out.println("setValue() throw ParserException");
                    throw (ParserException) value;

                } else {
                    System.out.println("setValue() return (Number)cell ParserException");
                    return (Number) cell.getValue();

                }
            } else if (type == Cell.NUMBER) {
                System.out.println("setValue() Cell.NUMBER");
                return (Number) cell.getValue();

                //     return  cell.getValue().toString();
            } else {
                System.out.println("setValue() Float(0)");
                return new Float(0);
            ...
            // a string or null
            //	return new Float(0);
            // System.out.println("setValue() #REFS?");
